...
def preprocDomains(alldomains,domains):
    ...
    Returns:
       sentdoms:
    """
    minnode,maxnode = min(start for start,end in alldomains), max(end for start,end in alldomains)
    sentdoms = set(domains)
    for start,end in domains:
        for spos,epos in itertools.product(range(max(minnode,start-1),min(maxnode,start+1)),range(max(minnode,end-1),min(maxnode,end+1))):
            if spos >= epos-1:
               continue 
            sentdoms.add((spos,epos))     
    return list(sentdoms)        
    #stlocs,endlocs = set(start for start,end in domains), set(end for start,end in domains)
    #sentdoms = set(domains)
...
def selectSubsetDomain(freqmat):
    """selects subset domains
    Args:
       freqmat:
    Returns:
       domains:   
    """ 
    posdoms = getPosDomains(freqmat,None)
    sentdoms = [(st,st+mylen-1) for mylen in [3] for st in xrange(np.shape(freqmat)[0]-mylen)]
    for mylen in [5,8]: #10
        sentdoms += [(st,st+mylen-1) for st in xrange(np.shape(freqmat)[0]-mylen) if random.random() < 0.2]
    return sentdoms
...
def makeDeconOutput(outprefix,domains,comp2dominds,comp2scale,metadata,VERBOSE=True):
    """make decon outputs
    Args:
       outprefix:
       domains:
    ...
    if VERBOSE:
       print "Writing deconvolution output to {0}".format(deconoutfile)
       print "Writing metadata to {0}".format(objoutfile)
