...
def runArmatus(filepath,maxscale,outprefix,armaoutfolder,ARMATUSPATH,topcount=1):
    """Runs Armatus
    Args:
       filepath:
       maxscale:
    ...
    code = "{0}/armatus -i {1} -g {2} -o {3}/{4} -k {5} -m".format(ARMATUSPATH,filepath,maxscale,armaoutfolder,outprefix,topcount)
    os.system(code)
    scale2doms = {}
...
def runArmatusConsensus(filepath,maxscale,outprefix,armaoutfolder,ARMATUSPATH,topcount=1):
    ...
    if not os.path.exists(armaoutfolder):
       os.makedirs(armaoutfolder)
    code = "{0}/armatus -i {1} -g {2} -o {3}/{4} -m -k {5}".format(ARMATUSPATH,filepath,maxscale,armaoutfolder,outprefix,topcount)
    os.system(code)
