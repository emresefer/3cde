...
def genMatlabScript(mscriptfile,sdpfile,outdatafile,SDPTDIR,curdir,runfolder):
    ...
    Args:
       ...
       runfolder: new run folder
    Returns:
    """
    with open(runfolder+"/"+mscriptfile,"w") as outfile:
        #outfile.write("cd('{0}')\n".format(SDPTDIR))
        outfile.write("cd('../{0}')\n".format(SDPTDIR))
...
def runSDPRelax(quadmat,runfolder,PATHS):
    ...
    """
    for key in PATHS:
        globals()[key] = PATHS[key]
    sdptfilename = "{0}/tempsdpt".format(runfolder)
    sdptoutfile = "{0}/sdptoutput.mat".format(runfolder)
    mscriptfile = "sdpt.m"
    tempoutfile = "{0}/tempim.txt".format(runfolder)
    genSDPAOut(quadmat,sdptfilename)
    olddir = os.getcwd()
    genMatlabScript(mscriptfile,sdptfilename,sdptoutfile,SDPTDIR,olddir,runfolder)
    code = "{0} -nodisplay -nosplash -nodesktop -r \" cd('{1}'); {2}; cd('{3}'); quit; \" '>' {4}".format(MATLABPATH,runfolder,mscriptfile.replace(".m",""),olddir,tempoutfile)
    os.system(code)
...
def coefs2BooleanNOTUSED(coefmat):
