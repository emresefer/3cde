...
def genObjStr(freqmat,compcount,domains,interdom,comp2sc,predomains,prob,algopar,kernmat):
    ...
    for dom in domains:
        ...
        if algopar["kernel"] == None:
           ...
           fsum = np.sum(np.multiply(kernmat[max(dom[0],dom[0]):min(dom[1],dom[1])+1,max(dom[0],dom[0]):min(dom[1],dom[1])+1],freqmat[max(dom[0],dom[0]):min(dom[1],dom[1])+1,max(dom[0],dom[0]):min(dom[1],dom[1])+1])) 
...
def genObjStrOptimal(freqmat,domains,comp2dominds,algopar,kernmat):
    """generates objective function string
    Args:
       freqmat: frequency matrix
       domains: all domains
       comp2dominds: current domain assignment
       algopar:
       kernmat:
    Returns:
       objstr: 
    """
    compcount = len(comp2dominds.keys())
    if algopar["kernel"] != None:
       coefest = lambda dom1,dom2: np.sum(kernmat[max(dom1[0],dom2[0]):min(dom1[1],dom2[1])+1,max(dom1[0],dom2[0]):min(dom1[1],dom2[1])+1])
    elif algopar["obj"] == "frobenius":
       coefest = lambda dom1,dom2: (min(dom1[1],dom2[1])-max(dom1[0],dom2[0]) + 1)**2
    ...
    objstr = " Minimize\n obj: " 
    singles, quads = [], []
    A = np.zeros((compcount,compcount),dtype=np.float64)
    for comp1,comp2 in itertools.combinations(range(compcount),2):
        domins1 = comp2dominds[comp1]
        domins2 = comp2dominds[comp2]
        qcoef = sum([coefest(domains[domin1],domains[domin2]) for domin1 in domins1 for domin2 in domins2 if intersect(domains[domin1],domains[domin2])])
        quads.append(" %.16f x{0} * x{1} ".format(comp1,comp2) %(4*qcoef))
        A[comp1,comp2] = qcoef
        A[comp2,comp1] = qcoef 
    for comp in xrange(compcount):
        domins = comp2dominds[comp]
        doms = [domains[domin] for domin in domins]
        qcoef = sum([coefest(dom,dom) for dom in doms])
        quads.append(" %.16f x{0} * x{0} ".format(comp) %(2*qcoef))
        A[comp,comp] = qcoef 
        if algopar["kernel"] == None:
           fsum = sum([np.sum(freqmat[dom[0]:dom[1]+1,dom[0]:dom[1]+1]) for dom in doms])
        else:
           fsum = sum([np.sum(np.multiply(kernmat[dom[0]:dom[1]+1,dom[0]:dom[1]+1],freqmat[dom[0]:dom[1]+1,dom[0]:dom[1]+1])) for dom in doms])  
        singles.append(" %.16f x{0} ".format(comp) %(-2.0*fsum))
    if len(singles) > 0:
       objstr += " ".join(singles)
    if len(quads) > 0:
       objstr += " + [ " + " + ".join(quads) + " ] "
...
def genBoundStrSecond(compcount):
    """generates optimal quadratic bounds
    Args:
       compcount: number of components 
    Returns:
       boundstr:
    """
    boundstr = " Bounds\n "
    boundstr += "\n"+ " ".join([" 0 <= x{0}\n".format(comp) for comp in xrange(compcount)])
    return boundstr
  
def runScaleOpt(comp2dominds,sideparams,fqsum):
    ...
    objstr = genObjStrOptimal(freqmat,domains,comp2dominds,algopar,kernmat)
    boundstr = genBoundStrSecond(compcount)
    outmethod = globals()["convertCplexOut"]
    runfolder = "delpbisubsecond"+''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(10))
    xdict, objval = EmbedUtilities.runCplexCode("",objstr,boundstr,"",runfolder,outmethod)
    objval += fqsum
    comp2scale = {comp:0.0 for comp in xrange(compcount)}
    for comp in xdict.keys():
        comp2scale[comp[0]] = xdict[comp]
    return comp2scale,objval    

ROUNDCOUNT = 25 
TESTMODE = False
def runBisub_frac(freqmat,domains,compcount,outprefix,prob,algopar,PATHS):
    ...
    if np.amax(freqmat) < 2.0*compcount:
       comp2scale = {comp:0.2*np.amax(freqmat)/float(compcount) for comp in comp2scale.keys()}
    objlist = [fqsum]
    sideparams = [compcount,freqmat,interdom,domains,predomains,prob,algopar,coefmat]
    unused,unobjval = {}, None
    while True:
       #first step
       ...
       objval += fqsum
       curcomp2dominds,step1objval = Round.roundCRfrac(xdict,sideparams,comp2scale,algopar,ROUNDCOUNT)
       if TESTMODE:
          ...
          if prob == "map":
             ...
             tempobjval += priorobj
          assert abs(tempobjval-step1objval) < 0.001
          assert TestfdBiSub.testLPData(sideparams,objstr)
          assert TestfdBiSub.testLPOutput(xdict,sideparams,cliques,objval,comp2scale,curcomp2dominds,algopar)
       if curobjval - step1objval >= 2:
          print "step1 ",step1objval 
          curobjval,comp2dominds = step1objval, deepcopy(curcomp2dominds)
          objlist.append(step1objval)
       else:
          unused,unobjval = curcomp2dominds, curobjval
          break
       curcomp2scale,step2objval = runScaleOpt(comp2dominds,sideparams,fqsum)
       if prob == "map":
       ...
       if TESTMODE:
          ...
          if prob == "map":
             tempobjval += priorobj
          assert abs(step2objval - tempobjval) < 0.5
       for comp in curcomp2scale.keys():
           if curcomp2scale[comp] < 0.01:
              curcomp2scale[comp] = 0   
       if curobjval - step2objval >= 2:
          ...
          break
    etime = time.time()
    if len(comp2dominds.keys()) == 0:
       comp2dominds = deepcopy(unused)
       objlist.append(unobjval)
    ...
    if TESTMODE:
       ...
       assert abs(curobjval - newobjval) < 0.5
...
def makeParser():
    ...
    parser.add_argument('-o', dest='outprefix', type=str, action='store', default='deconfracout', help='output prefix(default: deconfracout)')
    parser.add_argument('-p', dest='prob', type=str, action='store', default='map', help='Estimation type, ML or MAP prob(default: map)')
    parser.add_argument('-l', dest='lambdaval', type=float, action='store', default=1.0, help='Lambda for MAP(default: 1.0)')
...
if  __name__ =='__main__':
    ...
    runBisub_frac(freqmat,domains,compcount,outprefix,prob,algopar,PATHS)
